    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new PostListAdapter(new ArrayList<Post>(), new UIUtils.LoadMoreListener() {
            ...
            public void onLoadMore() {
                AsyncTaskCompat.executeParallel(new LoadLikePostTask(adapter.getDataCount() / Ins.COUNT_PER_PAGE + 1));
            }
        });
        recyclerView.setAdapter(adapter);
    }

    private class LoadLikePostTask extends AsyncTask<Void, Void, List<Post>> {

        int page;

        public LoadLikePostTask(int page) {
            this.page = page;
        }
        ...
        protected List<Post> doInBackground(Void... params) {
            try {
                return Ins.getLikedPostListSelf();
